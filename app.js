...
soynode.setOptions({
  tmpDir: '/tmp/zoy',
  allowDynamicRecompile: !IS_PRODUCTION,
...
router.get('/', routeDocumentRoot);
router.get('/api/list', routeListAPI);
router.all(/.*/, route404);
...
function getSoyData() {
  ...
  validateSoyData(soyDataCache);
  return soyDataCache;
}

function routeDocumentRoot(req, res) {
  res.writeHeader(200, {'content-type': 'text/html'});
  res.write(soynode.render('app.soy.index.main', getSoyData()));
  res.end();
}
...
function routeListAPI(req, res) {
  var t;
  var offset = (t = req.params['offset']) ? parseInt(t, 10) : 0;
  var count = (t = req.params['count']) ? parseInt(t, 10) : 50;
